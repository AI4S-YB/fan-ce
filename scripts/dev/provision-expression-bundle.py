...
from apps.datasets.bundle_provisioning import discover_expression_bundle, provision_expression_bundle
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Provision an expression/transcriptome bundle into the Dataset platform.")
    parser.add_argument("--bundle-dir", required=True, help="Bundle directory, for example /path/to/transcriptome")
    parser.add_argument("--title", help="Dataset title. Defaults to the bundle metadata title or directory name.")
    ...
    parser.add_argument("--reference-version-id", type=int, help="Optional reference genome dataset version id for quantified_against lineage.")
...
def main() -> int:
    ...
    with MyDBManager() as db:
        result = provision_expression_bundle(
            db=db,
