...
from apps.datasets.bundle_provisioning import discover_variome_bundle, provision_variome_bundle
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Provision a variome/VCF bundle into the Dataset platform.")
    parser.add_argument("--bundle-dir", required=True, help="Bundle directory, for example /path/to/variome")
    parser.add_argument("--title", help="Dataset title. Defaults to the selected primary file stem.")
    ...
    parser.add_argument("--primary-file", help="Optional explicit primary VCF/BCF file inside the bundle.")
    parser.add_argument("--reference-version-id", type=int, help="Optional reference genome dataset version id for called_against lineage.")
...
def main() -> int:
    ...
    with MyDBManager() as db:
        result = provision_variome_bundle(
            db=db,
            ...
            assembly=args.assembly,
            primary_file=args.primary_file,
            reference_version_id=args.reference_version_id,
