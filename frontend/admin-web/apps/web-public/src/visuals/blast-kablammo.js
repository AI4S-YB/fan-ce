...
export function createKablammo(container, data, _options) {
  render(container, data, _options);

  const onResize = debounce(() => render(container, data, _options), 200);
...
function render(container, data, _options) {
  container.innerHTML = '';

  const { id: hitId, length: sLen, hsps } = data;
  if (!hsps || hsps.length === 0) return;

  const onHspClick = _options?.onHspClick || null;
  const seqType = _options?.seqType || 'amino_acid';
  ...
  g.append('g').call(d3.axisTop(xQ).tickFormat(d => formatTick(d, seqType)).ticks(5))
    .selectAll('text').style('font-size', '9px');

  // Subject axis (bottom)
  const xS = d3.scaleLinear().domain([0, subjectLen]).range([0, width]);
  g.append('g').attr('transform', `translate(0,${innerH})`)
    .call(d3.axisBottom(xS).tickFormat(d => formatTick(d, seqType)).ticks(5))
    .selectAll('text').style('font-size', '9px');
  ...
  for (let hi = 0; hi < hsps.length; hi++) {
    const hsp = hsps[hi];
    ...
    const poly = g.append('polygon')
      ...
      .on('click', function () {
        if (onHspClick) onHspClick(hsp, hi);
      })
      .append('title')
