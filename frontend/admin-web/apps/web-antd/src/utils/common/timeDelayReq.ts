...
type ErrorDataFunc = (param: any, error: any, reject: any) => AsyncResult;
...
export interface InitOption {
  ...
  cacheKey?: CacheKeyFunc;
  delay?: number;
  getErrorData?: (param: any, error: any, reject: any) => AsyncResult;
  isUseCache?: boolean;
}
// 延时请求工具类
export class TimeDelayReq {
  ...
  constructor(initOption: InitOption) {
    const { api, getErrorData, cacheKey, cacheTime, delay, isUseCache } =
      initOption;
    this.api = api;
  ...
  async _loadByParams(paramList: Array<Recordable>) {
    ...
    try {
      const res: Map<string, DelayResult> = await this.api(
        { formatParamList },
        this.cacheKey,
      );
  ...
  _loadFromApi(
    param: any,
    resolve: (value: AsyncResult | PromiseLike<AsyncResult>) => void,
