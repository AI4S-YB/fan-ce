...
export function findNodeByKey(
  ...
  for (const item of tree) {
    ...
    if (item[children]) {
      findNodeByKey(key, item[children]);
    }
  }
  return {};
}
...
// 根据父id查找子节点的id
export function findChildrenByParentId<T = number | string, N = Recordable>(
  nodeKey: T,
  treeData: N[] = [],
  config: Partial<TreeHelperConfig> = {},
): T[] {
...
export function findAllKeys<N = Recordable>(
  treeData: N[],
  config: Partial<TreeHelperConfig> = {},
) {
  const keys: any[] = [];
...
// 根据父id查找子节点
export function getById<T = number | string, N = Recordable>(
  nodeKey: T,
  treeData: N[],
  config: Partial<TreeHelperConfig> = {},
): N {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    childrenField &&
      ...
      list.splice(i + 1, 0, ...list[i][childrenField]);
  }
  return {} as N;
}
