...
type ResolvePropType<T> =
  ResolveProp<T> extends { type: infer V } ? V : ResolveProp<T>;
type ResolvePropTypeWithReadonly<T> =
  Readonly<T> extends Readonly<Array<infer A>>
    ? ResolvePropType<A[]>
    : ResolvePropType<T>;
...
export interface BuildPropOption<T, D extends BuildPropType<T, V, C>, R, V, C> {
  ...
  default?: R extends true
    ? never
    : D extends Array<any> | Record<string, unknown>
      ? () => D
      : (() => D) | D;
...
type _BuildPropDefault<T, D> = [T] extends [
  Array<any> | Fn | Record<string, unknown>,
]
  ? D
  : D extends () => T
    ? ReturnType<D>
...
export type BuildPropReturn<T, D, R, V, C> = BuildPropDefault<
  BuildPropType<T, V, C>,
  IfUnknown<D, never>,
  IfUnknown<R, false>
> & {
...
type NativePropType = [
  ((...args: any) => any) | null | undefined | { new (...args: any): any },
];
...
export const keyOf = <T extends object>(arr: T) =>
