...
const props = defineProps({
  ...
  text: propTypes.string.def($t('component.upload.upload')),
...
const bindProps = computed(() => {
  ...
  // 如果当前是图片上传模式，就只能上传图片
  if (isImageMode.value && !bind.accept) bind.accept = 'image/*';
...
watch(
  ...
  (val) => {
    if (Array.isArray(val)) {
      if (props.returnUrl) parsePathsValue(val.join(','));
      else parseArrayValue(val);
    } else {
      parsePathsValue(val);
    }
  },
...
// 解析数据库存储的逗号分割
function parsePathsValue(paths: null | string | undefined) {
...
// 解析数组值
function parseArrayValue(array: any[] | string) {
...
function onBeforeUpload(file: any) {
  ...
  if (isImageMode.value && !file.type.includes('image')) {
    createMessage.warning($t('component.uploadExtra.pleaseUploadImage'));
...
function onRemove() {
  if (props.removeConfirm) {
    return new Promise((resolve) => {
      createConfirm({
        title: $t('component.upload.del'),
        content: isImageMode.value
          ? $t('component.uploadExtra.deleteConfirmImage')
          : $t('component.uploadExtra.deleteConfirmFile'),
        iconType: 'warning',
        onOk: () => resolve(true),
...
function onFileChange(info: any) {
  if (!info.file.status && uploadGoOn.value === false) info.fileList.pop();
  ...
  if (info.file.status === 'done') {
    ...
    if (info.file.response.code === 0) {
      successFileList = fileListTemp.map(
        (file: { response: { data: any }; url: any }) => {
          if (file.response) {
      ...
      successFileList = fileListTemp.filter((item: { uid: any }) => {
        return item.uid !== info.file.uid;
      });
      createMessage.error($t('component.uploadExtra.uploadFailedName', { name: info.file.name }));
    }
...
function uidGenerator() {
  return `-${Number.parseInt((Math.random() * 10_000 + 1, 10).toString())}`;
}

function getFileName(path: any) {
  if (path.includes('\\')) {
    const reg = /\\/g;
...
<template>
  <div ref="containerRef" :class="`${prefixCls}-container`">
    ...
    >
      ...
      <Button v-else-if="buttonOpen" :disabled="isMaxCount || disabled">
