...
export function useFormEvents({
  ...
  async function setFieldsValue(values: Recordable): Promise<void> {
    ...
    // key 支持 a.b.c 的嵌套写法
    const delimiter = '.';
    const nestKeyArray = fields.filter((item) =>
      String(item).includes(delimiter),
    );
      ...
      if (hasKey || !!constructValue) {
        ...
        if (itemIsDateType(key)) {
          if (Array.isArray(fieldValue)) {
            const arr: any[] = [];
            for (const ele of fieldValue) arr.push(ele ? dateUtil(ele) : null);
  ...
  async function appendSchemaByField(
    ...
    else if (index !== -1) schemaList.splice(index + 1, 0, ..._schemaList);
...
function getDefaultValue(
  ...
  if (isInput) return defaultValue || '';
