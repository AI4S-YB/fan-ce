...
type ImageUploadType = 'picture' | 'picture-card' | 'text';
...
function changeFileValue(value: any) {
  ...
  if (stateStr !== fileState.oldStr) {
    ...
    if (props.multiple) {
      if (!isNil(value)) {
        if (isArray(value)) list = value as string[];
        else list.push(value as string);
      }
    } else {
      if (!isNil(value)) list.push(value as string);
    }
...
const handleBeforeUpload: UploadProps['beforeUpload'] = (file) => {
  ...
  if (!isPNG)
    message.error(
      t('component.upload.acceptUpload', [props.fileType.toString()]),
    );
  ...
  if (!(isPNG && isLt2M)) fileList.value.pop();
