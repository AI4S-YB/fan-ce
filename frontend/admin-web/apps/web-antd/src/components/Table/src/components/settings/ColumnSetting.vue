...
watchEffect(() => {
  ...
  setTimeout(() => {
    if (isSetColumnsFromThis) isSetColumnsFromThis = false;
    else if (columns.length > 0) init();
  }, 0);
});

watchEffect(() => {
  const values = unref(getValues);
  if (isSetPropsFromThis) isSetPropsFromThis = false;
  else cacheTableProps = cloneDeep(values);
...
async function init(isReset = false) {
  ...
  if (columnListEl && (columnListEl as any).$el) {
    ...
    [...el.children].forEach((item) => el.removeChild(item));
  }
  await nextTick();
  ...
  nextTick(() => {
    ...
    sortable = Sortablejs.create(unref(el), {
      ...
      onEnd: (evt) => {
        ...
        setColumns(
          columns.filter((item) => state.checkedList.includes(item.value)),
        );
      },
    });
    // 记录原始order 序列
...
function handleColumnFixed(item: BasicColumn, fixed?: 'left' | 'right') {
  ...
  if (index !== -1) columns[index].fixed = isFixed;
...
function updateSortOption(column: BasicColumn) {
  plainSortOptions.value.forEach((item) => {
    if (item.value === column.dataIndex) Object.assign(item, column);
  });
}
</script>
