...
export function useDataSource(
  ...
  function setTableKey(items: any[]) {
    ...
    items.forEach((item) => {
      ...
      if (item.children && item.children.length > 0) setTableKey(item.children);
    });
  }
  ...
  const getDataSourceRef = computed(() => {
    ...
    if (!dataSource || dataSource.length === 0) return unref(dataSourceRef);
    ...
    if (unref(getAutoCreateKey)) {
      ...
      if (
        firstItem &&
        lastItem &&
        (!firstItem[ROW_KEY] || !lastItem[ROW_KEY])
      ) {
  ...
  async function updateTableData(index: number, key: string, value: any) {
    ...
    if (record) dataSourceRef.value[index][key] = value;
