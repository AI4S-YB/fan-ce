...
export default defineComponent({
  ...
  setup(props: any) {
    ...
    function upEditDynamicDisabled(
      record: any,
      column: { dataIndex: any; key: any },
      value: any,
    ) {
    ...
    async function handleChange(e: any) {
      const component = unref(getComponent);
      if (!e) currentValueRef.value = e;
      else if (component === 'Checkbox')
        currentValueRef.value = e.target.checked;
      else if (component === 'Switch') currentValueRef.value = e;
    ...
    function onClickOutside() {
      if (props.column?.editable || unref(getRowEditable)) return;
    ...
    function initCbs(cbs: 'cancelCbs' | 'submitCbs' | 'validCbs', handle) {
      if (props.record) {
