...
function handleItem(item: BasicColumn, ellipsis: boolean) {
  ...
  if (ellipsis) {
    if (!key)
      item.key =
        typeof dataIndex === 'object' ? dataIndex.join('-') : dataIndex;
  ...
  if (children && children.length > 0) handleChildren(children, !!ellipsis);
}
...
export function useColumns(
  ...
  const getViewColumns = computed(() => {
    ...
    const mapFn = (column: any) => {
      ...
      const isDefaultAction = [ACTION_COLUMN_FLAG, INDEX_COLUMN_FLAG].includes(
        flag!,
      );
   ...
   */
  function setColumns(
    columnList: (string | string[])[] | Partial<BasicColumn>[],
  ) {
  ...
  function getColumns(opt?: GetColumnsParams) {
    ...
    if (sort) columns = sortFixedColumn(columns);
