...
setupVbenVxeTable({
  configVxeTable: (vxeUI) => {
    ...
    vxeUI.renderer.add('DictTagStatus', {
    ...
    vxeUI.formats.add('formatDateTime10', {
      tableCellFormatMethod({ cellValue }) {
        return dayjs(cellValue ? cellValue * 1000 : cellValue).format(
          'YYYY-MM-DD',
        );
      },
    });
    vxeUI.formats.add('formatBaseStatus', {
      tableCellFormatMethod({ cellValue }) {
        return cellValue === '1' ? '可用' : '禁用';
      },
    });
