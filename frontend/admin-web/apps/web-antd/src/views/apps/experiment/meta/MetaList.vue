...
import {
  delExperimentMetaApi,
  getExperimentMetaListApi,
...
function onDelete(row: any) {
  const hideLoading = message.loading({
    content: $t('system.experiment.deleting', { key: row.key }),
    duration: 0,
    key: 'action_process_msg',
  });
  delExperimentMetaApi({ ids: [row.id] })
    .then(() => {
      ...
      message.success({
        content: $t('system.experiment.deleteSuccess', { key: row.key }),
        key: 'action_process_msg',
      });
    })
    .catch(() => {
...
function onActionClick(e: OnActionClickParams<any>) {
  switch (e.code) {
    case 'add': {
      const row = e.row;
      row.experiment_id = prop.rawData.id;
...
const childGridOptions = reactive<VxeGridProps<any>>({
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }) => {
        return await getExperimentMetaListApi({
          page: page.currentPage,
          size: page.pageSize,
          experiment_id: prop.rawData.id,
        });
      },
    },
  },
