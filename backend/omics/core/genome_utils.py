...
def delete_genome_db(conn):
    ...
    try:
        ...
        for table in tables:
            table_name = table[0]
            # Validate identifier before DDL (SQLite does not support parameterized DDL)
...
"""
async def process_genome_background(path: str, task_id: str, db: Session):
    try:
        # Extract metadata
        meta = extract_genome_metadata(path)
        
        # Create new genome entry
        new_genome = GenomeMetadata(**meta)
        db.add(new_genome)
        db.commit()
        
        # Update task status
        cache.set(f"genome_task_{task_id}", {
            "status": "completed",
            "message": "Genome processing completed",
            "genome_id": new_genome.id
        })
...
async def process_genome_files(task_id, genome_path, operation='insert'):
    """
    Main function to handle genome operations
    
    Args:
        genome_path (str): Path to the genome folder
        operation (str): Operation to perform (insert, delete). Default: insert
    """

    # Check if genome_path is absolute
    if not os.path.isabs(genome_path):
        raise ValueError(f"Genome path must be absolute. Current path: {genome_path}")

    # check if genome_path exists    
    check_file_access(genome_path)
    ...
    try:
        ...
        error_msg = f"Error accessing genome info files: {str(e)}"
    ...
    try:
        ...
        if operation == 'insert':
            ...
            logger.info(f"Successfully inserted genome data for {genome_path}")
            print(f"Successfully insert genome data for {genome_path}")
            
        elif operation == 'delete':
            delete_genome_db(conn)
            logger.info(f"Successfully deleted genome data for {genome_path}")
        ...
        error_msg = f"Error processing genome operation: {str(e)}"
        logger.error(error_msg)
