...
class VcfExtractor(BaseExtractor):
    ...
    def __init__(self):
        # 直接指定模板路径
        template_path = "vcf.json"
        super().__init__(template_path)
        if not PYSAM_AVAILABLE:
            raise ImportError("pysam is required for VCF extraction")
    
    def _variant_type(self, ref: str, alts: List[str]) -> str:
        """
        根据 ref/alt 判断变异类型：
        - snv: 单核苷酸多态
        - indel: 插入/删除
    ...
    def _get_generic(self, hdr, key: str):
    ...
    def extract(self, path: str, sniff: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 重置计数字段为None，只在有数据时才设置
        self.numRecords = None
        self.variantTypes = None
        self.missingGenotypeTotal = None
        self.variantDensityPerContig = None
        
        # 初始化计数器
        num_records = 0
        variant_types = {"snv": 0, "indel": 0, "multiallelic": 0, "other": 0}
        missing_gt_total = 0
        ...
        # 通用键
        self.fileDate = self._get_generic(hdr, "fileDate")
        self.source = self._get_generic(hdr, "source")
        ...
        for rec in vf.fetch():
            ...
            # 变异类型统计
            vt = self._variant_type(rec.ref or "", list(rec.alts or []))
            variant_types[vt] += 1
        ...
        if num_records > 0:
            ...
            self.variantTypes = variant_types
            self.missingGenotypeTotal = missing_gt_total
        
        # variantDensityPerContig只在有变异时设置
        if variant_density:
...
# Register the extractor with the factory if pysam is available
if PYSAM_AVAILABLE:
    register_extractor("VCF", VcfExtractor())