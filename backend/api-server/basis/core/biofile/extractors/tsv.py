...
class TsvExtractor(BaseExtractor):
    """
    TSV文件提取器
    提取TSV文件中的列名、行数、分隔符等信息
    """
    
    def __init__(self):
        template_path = "tsv.json"
    ...
    def _has_header(self, path: str, encoding: str) -> bool:
        """检测是否有表头"""
        try:
    ...
    def extract(self, path: str, sniff: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 检测是否有表头
            has_header = self._has_header(path, encoding)
