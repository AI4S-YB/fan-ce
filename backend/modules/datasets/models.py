...
class AssetTypeRegistry(Base):
    ...
    is_system = Column(Boolean, default=False, comment="是否系统内置")
    is_active = Column(Boolean, default=True, comment="是否启用")
    engine = Column(String(64), comment="查询引擎标识")
    sort_order = Column(Integer, default=0, comment="排序")
    meta_json = Column(JSONB, comment="扩展元数据")
    create_time = Column(DateTime(timezone=True), comment="创建时间")
    update_time = Column(DateTime(timezone=True), comment="更新时间")
...
class DatasetScanRoot(Base):
    ...
    root_path = Column(Text, unique=True, index=True, comment="根目录路径")
...
class DatasetScanJob(Base):
    ...
    id = Column(Integer, primary_key=True)
    root_id = Column(Integer, ForeignKey("dataset_scan_root.id", ondelete="RESTRICT"), index=True, comment="扫描目录 ID")
...
class DatasetStagingFile(Base):
    ...
    storage_uri = Column(Text, comment="统一存储 URI")
    local_path = Column(Text, comment="本地暂存路径")
    ...
    scan_root_id = Column(Integer, ForeignKey("dataset_scan_root.id", ondelete="SET NULL"), index=True, nullable=True, comment="来源扫描目录 ID")
    scan_job_id = Column(Integer, ForeignKey("dataset_scan_job.id", ondelete="SET NULL"), index=True, nullable=True, comment="来源扫描任务 ID")
    relative_path = Column(Text, comment="相对扫描目录路径")
    ...
    linked_dataset_id = Column(Integer, ForeignKey("dataset_registry.id", ondelete="SET NULL"), index=True, nullable=True, comment="已注册关联的 dataset id")
...
class DatasetWorkflowTask(Base):
    ...
    id = Column(Integer, primary_key=True)
    dataset_id = Column(Integer, ForeignKey("dataset_registry.id", ondelete="RESTRICT"), index=True, comment="关联 dataset ID")
...
class DatasetPublishRecord(Base):
    ...
    id = Column(Integer, primary_key=True)
    dataset_id = Column(Integer, ForeignKey("dataset_registry.id", ondelete="RESTRICT"), index=True, nullable=True, comment="关联 dataset ID")
...
class AssetFile(Base):
    ...
    id = Column(Integer, primary_key=True)
    dataset_asset_id = Column(Integer, ForeignKey("dataset_asset.id", ondelete="RESTRICT"), index=True, comment="dataset asset ID")
    ...
    index_of_file_id = Column(Integer, ForeignKey("asset_file.id", ondelete="SET NULL"), index=True, nullable=True, comment="索引对应主文件 ID")
...
class DatasetLineageEdge(Base):
    ...
    id = Column(Integer, primary_key=True)
    src_dataset_version_id = Column(Integer, ForeignKey("dataset_version.id", ondelete="RESTRICT"), index=True, comment="来源版本 ID")
    src_asset_id = Column(Integer, ForeignKey("dataset_asset.id", ondelete="SET NULL"), index=True, nullable=True, comment="来源资产 ID")
    dst_dataset_version_id = Column(Integer, ForeignKey("dataset_version.id", ondelete="RESTRICT"), index=True, comment="目标版本 ID")
    dst_asset_id = Column(Integer, ForeignKey("dataset_asset.id", ondelete="SET NULL"), index=True, nullable=True, comment="目标资产 ID")
...
class FunctionalGene(Base):
    ...
    version_id = Column(Integer, ForeignKey("dataset_version.id", ondelete="RESTRICT"), index=True, comment="dataset version ID")
    asset_id = Column(Integer, ForeignKey("dataset_asset.id", ondelete="RESTRICT"), index=True, comment="functional_annotation asset ID")
...
class PhenomeImportRun(Base):
    ...
    dataset_id = Column(Integer, ForeignKey("dataset_registry.id", ondelete="RESTRICT"), index=True, comment="dataset ID")
    version_id = Column(Integer, ForeignKey("dataset_version.id", ondelete="RESTRICT"), index=True, comment="dataset version ID")
    asset_id = Column(Integer, ForeignKey("dataset_asset.id", ondelete="RESTRICT"), index=True, comment="phenome asset ID")
    source_file_path = Column(Text, comment="source file path")
...
class PhenomeSubject(Base):
    ...
    version_id = Column(Integer, ForeignKey("dataset_version.id", ondelete="RESTRICT"), index=True, comment="dataset version ID")
    asset_id = Column(Integer, ForeignKey("dataset_asset.id", ondelete="RESTRICT"), index=True, comment="phenome asset ID")
    import_run_id = Column(Integer, ForeignKey("phn_import_run.id", ondelete="RESTRICT"), index=True, comment="import run ID")
...
class PhenomeSourceColumn(Base):
    ...
    trait_id = Column(Integer, ForeignKey("phn_trait.id", ondelete="RESTRICT"), index=True, comment="trait ID")
    trait_code = Column(String(128), index=True, comment="trait code")
